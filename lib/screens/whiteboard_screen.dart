...
class _WhiteboardScreenState extends State<WhiteboardScreen> {
  ...
  Future<void> _downloadWhiteboard() async {
    try {
      RenderRepaintBoundary boundary =
          _whiteboardKey.currentContext!.findRenderObject() as RenderRepaintBoundary;
      ui.Image image = await boundary.toImage(pixelRatio: 2.0);
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Whiteboard downloaded to: $filePath")),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
...
Future<void> _exportAndShareWhiteboard() async {
        ...
        content: Column(
          ...
          children: [
            Slider(
              ...
              label: (isEraser ? eraserWidth : strokeWidth).toStringAsFixed(1),
              onChanged: (value) {
                setState(() {
                  if (isEraser) {
                    eraserWidth = value;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(currentImagePath != null ? "Edit Whiteboard" : "New Whiteboard"),
        actions: [
          ...
          PopupMenuButton<String>(
            onSelected: (value) {
              ...
              if (value == 'Download') _downloadWhiteboard(); // Added download option
            },
            itemBuilder: (context) => [
              const PopupMenuItem(value: 'Save', child: Text('Save Whiteboard')),
              const PopupMenuItem(value: 'Export', child: Text('Share Whiteboard')),
              const PopupMenuItem(value: 'Import', child: Text('Import Image')),
              const PopupMenuItem(value: 'Download', child: Text('Download Whiteboard')), // Added download option
            ],
          ),
        ],
      ),
            ...
            child: RepaintBoundary(
              ...
              child: GestureDetector(
                ...
                onPanEnd: (_) => setState(() {
                  ...
                  currentPage.add(DrawingPoint(Offset.zero, Colors.transparent, 0));
                }),
                child: CustomPaint(
                  painter: WhiteboardPainter(currentPage, backgroundImage),
                  size: Size(MediaQuery.of(context).size.width, pageHeight),
                ),
              ),
            ),
          ),
...
class WhiteboardPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < points.length - 1; i++) {
      ...
      if (points[i].color == Colors.transparent || points[i + 1].color == Colors.transparent) {
        continue;
      }
